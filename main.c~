...
static int __init batadv_init(void)
{
	...
	//printk(KERN_ALERT "%pM\n",dev->dev_addr);
	
	//struct in_device *in_dev = in_dev_get(dev);	
	//printk(KERN_ALERT "%pI4\n",&(in_dev->ifa_list->ifa_address)); // is the IPv4 address
...
{
//   printk(KERN_ALERT "Go to batadv_batman_skb_recv function!\n");
	...
	if(idx!=0)
	  printk(KERN_ALERT "[PhuTran] Receive packet: %i (type)\n",idx);
...
static void batadv_recv_handler_init(void)
{
	...
	BUILD_BUG_ON(sizeof(struct batadv_ogm_packet) != 28);
