...
{
  printk("dz >> vao ham batadv_nc_init\n");
	int ret;
...
{
  printk("dz >> send non-coded packet\n");
				    ...
				    struct batadv_ogm_packet *ogm_packet)
{
	...
	if (ogm_packet->tq < bat_priv->nc.min_tq)
		return false;
  printk("dz >> check xem co nc dc ko >> true\n");
	return true;
}
			      ...
			      int is_single_hop_neigh)
{
	...
	if (!batadv_can_nc_with_orig(bat_priv, orig_node, ogm_packet) && !is_single_hop_neigh)
		goto out;

	/* Add orig_node as in_nc_node on hop */
	in_nc_node = batadv_nc_get_nc_node(bat_priv, orig_node,orig_neigh_node, true);
	if (!in_nc_node)
		goto out;
	...
	out_nc_node = batadv_nc_get_nc_node(bat_priv, orig_neigh_node,orig_node, false);
	if (!out_nc_node)
		goto out;
				   ...
				   struct batadv_neigh_node *neigh_node)
{
  ...
  printk("dz >> vao ham coded_packet\n");
					 ...
					 struct sk_buff *skb)
{
  ...
  printk("dz >> skb_store_sniffed_unicast (luu lai goi tin unicast lay dc trong promisc mode)\n");
	struct ethhdr *ethhdr = eth_hdr(skb);
